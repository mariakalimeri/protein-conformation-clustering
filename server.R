shinyServer(function(input, output, session) {
  ...
  observe({
    if (input$browsePdb == 0) {
      ...
      updateTextInput(session, "pdbpath", value = file_choose2())
    }
  })
  ...
  observe({
    if (input$browseTrj == 0) {
      ...
      updateTextInput(session, "trjpath", value = file_choose2())
    }
  })
  ...
  clusteringOutput <- eventReactive(input$goButton, {
    validate(
      ...
      need(input$dcdFrequency != "", paste(
        "Please define the damping",
        "frequency of the dcd trajectory in ns"
      )),
      need(input$chainId != "", paste(
        "Please choose a chain identifier",
    ...
    validate(
      need(
        substr(
          input$trjpath, nchar(input$trjpath) - 3,
          nchar(input$trjpath)
        ) == ".dcd",
      ...
      need(
        substr(
          input$pdbpath, nchar(input$pdbpath) - 3,
          nchar(input$pdbpath)
        ) == ".pdb",
    ...
    withProgress(message = "Reading input", value = 0, {
      ...
      validate(
        need(
          nrow(pdb$atom) == as.numeric(nAtomsIndcd),
    ...
    withProgress(message = "Doing the clustering", value = 0, {
      clusteringOutput <- cluster_function_gui(input$trjpath, input$cutoff,
  ...
  output$clusterMemberships <- renderPlot({
    ...
    plot(timeVector, clusteringOutput()$membershipVector,
      type = "p",
  ...
  output$downloadMeM <- downloadHandler(
    ...
    content = function(con) {
      write.table(clusteringOutput()$membershipVector, con,
        row.names = F,
        col.names = F
      )
    }
  ...
  output$vectorCluster <- renderPlot({
    totalNoOfFrames <- clusteringOutput()$nframes
    timeVector <- seq(1, totalNoOfFrames) * as.numeric(input$dcdFrequency)
    plot(timeVector, clusteringOutput()$vectorCluster,
      pch = ".",
      cex.axis = 1.4, cex.lab = 1.4, cex.main = 1.7, col = "black",
  ...
  output$downloadClus <- downloadHandler(
    ...
    content = function(con) {
      write.table(clusteringOutput()$vectorCluster, con,
        row.names = F,
        col.names = F
      )
    }
    ...
    tryCatch(
      {
        m.e <- nls(yValues ~ I(a * (1 - exp(1)^(-timeVector / b))),
          data = mydata, start = list(
            a = input$NGuess,
            b = input$charTimeGuess
          ), trace = T
        )
        ...
        output$charactTime2 <- renderText({
          charactTime
        })
        output$N_inf <- renderText({
          N_inf
        })
        output$N_inf2 <- renderText({
          N_inf
        })
  ...
  output$trjfileName2Display <- renderText({
    tail(strsplit(
      input$trjpath,
      "/"
    )[[1]], 1)
  ...
  output$pdbfileName2Display <- renderText({
    tail(strsplit(
      input$pdbpath,
      "/"
    )[[1]], 1)
  ...
  output$clusterAtoms <- renderText({
    input$atomTypes
  })

  output$clusteringCutoff <- renderText({
    input$cutoff
  })
  ...
  writePDBfunc <- function(coN) {
    ...
    pdb <- read.pdb(input$pdbpath)
    selectedAtomsProt <- atom.select.pdb(pdb,
      string = "protein",
      chain = input$chainId
    )
    xyzCoords <- clusteringOutput()$xyzLeaders
    write.pdb(NULL,
      file = coN, xyz = xyzCoords[1, ],
  ...
  observeEvent(input$renderButton, {
    withProgress(message = "Rendering images", value = 0, {
      ...
      output$renderingComplete <- renderText({
        "Rendering completed!"
      })
      output$renderedImage1 <- renderImage(
        {
          list(
            src = "www/renderedImage1.tga",
  ...
  myG <- eventReactive(input$drawGraph, {
    ...
    # enrichedGraph <- create_graph_from_membership_vector(membershipVectorFromFile)
    enrichedGraph <-
      create_graph_from_membership_vector(clusteringOutput()$membershipVector)
  ...
  output$networkOfStates <- renderPlot({
    myGplotted()
  })
  ...
  myKineticG <- eventReactive(input$drawKineticGraph, {
    ...
    # Write temp file
    write.table(myG()$graphAsWeightedEdgeList,
      file = "mclInput.txt", col.names = F,
      row.names = F
    )
    system(paste("mcl mclInput.txt --abc -I", input$inflation, " -o mclOut.txt"))
    kinMembershipVector <-
      convert_to_kinetic_membership_vector(clusteringOutput()$membershipVector)
    # For debugging of Step 3
    # fileName <- "sampleFiles_dcd_n_pdb/membershipVector.dat"
    # membershipVectorFromFile <- as.matrix(read.table(fileName))
    # kinMembershipVector <-
    #     convert_to_kinetic_membership_vector(membershipVectorFromFile)
    system("rm mclInput.txt mclOut.txt")
    tryCatch(
      {
        myKinG <- create_graph_from_membership_vector(kinMembershipVector)
        myKinG <- myKinG$myG
        coKin <- eval(parse(text = input$drawingKinAlg))(myKinG)
      },
      error = function(cond) {
        stop({
          "Coarse-graining was not possible. Try increasing the inflation 
  ...
  myKinGplotted <- function() {
    ...
    desiredKinInterval <- c(
      as.numeric(input$minNodeKinSize),
  ...
  output$kineticnetworkOfStates <- renderPlot({
    myKinGplotted()
  })
