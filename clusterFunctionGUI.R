clusterFunctionGUI <- function (trjfile, cutoff, fittingInds, outputProtInds, verbose = TRUE, cell = FALSE){
    ...
    dcd.frame <- function(trj, head, cell) {
        if (head$charmm && head$extrablock) {
            ...
            u <- readBin(trj, "numeric", size = 8, n = (a/8), 
                         endian = head$end)
            a <- readBin(trj, "integer", 1, endian = head$end)
        }
        if (head$nfixed == 0) {
            a <- readBin(trj, "integer", 1, endian = head$end)
            x <- readBin(trj, "numeric", size = 4, n = (a/4), 
                         endian = head$end)
            a <- readBin(trj, "integer", 1, endian = head$end)
            a <- readBin(trj, "integer", 1, endian = head$end)
            y <- readBin(trj, "numeric", size = 4, n = (a/4), 
                         endian = head$end)
            a <- readBin(trj, "integer", 1, endian = head$end)
            a <- readBin(trj, "integer", 1, endian = head$end)
            z <- readBin(trj, "numeric", size = 4, n = (a/4), 
                         endian = head$end)
            a <- readBin(trj, "integer", 1, endian = head$end)
        }
        else {
    ...
    for (i in 2:nframes) {
        ...
        if (curr.pos <= head$end.pos) {
            ...
            ## FITTING ##
            fitted.coords <- fit.xyz(fixed = coordsFirstFrame, mobile = coords, 
                                     fixed.inds=fittingInds,
                                     mobile.inds=fittingInds)
            
            ## RMSD Calculation of frame from all existing leaders
            rmsdValues <- rmsd(a=fitted.coords, b=leaders, a.inds=fittingInds, 
...
dcd.header <- function(trj, verbose=TRUE, ...) {
  ...
  if (class(ll) == "try-error") {
    cur.pos <- seek(trj, where = (80 * ntitle + cur.pos), 
                    origin = "start")
  }
  a <- readBin(trj, "integer", 1, endian = end)
  a <- readBin(trj, "integer", 1, endian = end)
  ...
  header <- list(natom = natom, nframe = nframe, first = first, 
